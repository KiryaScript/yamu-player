...
const electron=require('electron'),node_crypto=require('node:crypto'),node_os=require('node:os');
let uuid;
try { uuid = require('uuid'); } catch(e) {
  uuid = { v4: () => node_crypto.randomUUID ? node_crypto.randomUUID() : 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, c => { const r = Math.random()*16|0, v = c == 'x' ? r : (r&0x3|0x8); return v.toString(16); }) };
}
let Store;
try { Store = require('electron-store'); } catch(e) { Store = class { get() {} set() {} }; }const appConfig={appHostname:"desktop"};const buildInfo={VERSION:"5.116.3",BRANCH:"9514e66969bd27e00a6f2403bbaf4675da866692"};const config = {
  app: appConfig,
...
electron.contextBridge.exposeInMainWorld("yandexMod", {
  ...
  getUserAccount: () => electron.ipcRenderer.invoke('mod:get-user-account'),
  getUserAccountSync: () => electron.ipcRenderer.sendSync('mod:get-user-account-sync'),
  onDownloadError: (cb) => {
...
window.document.addEventListener("DOMContentLoaded", () => {
  ...
  if (isApplicationHostname(window.location.hostname)) {
    ...
    // Inject Mod Styles and Client Script Safely (Only in main music application, not in auth webview)
    try {
